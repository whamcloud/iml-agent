...
RPM_SOURCES   := $(shell spectool $(RPM_DIST_VERSION_ARG) \
		   -l $(RPM_SPEC) | sed -e 's/.*\///')
...
$(shell { echo 'VERSION = "$(VERSION)"';                                \
	  ...
	  if ! cmp scm_version.py.tmp chroma_agent/scm_version.py; then \
	      cp scm_version.py.tmp chroma_agent/scm_version.py;        \
	  fi)
...
deps: chroma_agent.egg-info/SOURCES.txt
...
chroma_agent.egg-info/SOURCES.txt:
